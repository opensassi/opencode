...
if [ -d "$FLAMEGRAPH_DIR/.git" ]; then
    ...
    git -C "$FLAMEGRAPH_DIR" fetch --tags --depth=1 2>/dev/null || true
    git -C "$FLAMEGRAPH_DIR" checkout v1.0 2>/dev/null || {
        echo "[WARN] Could not checkout v1.0; resetting to tag..."
        git -C "$FLAMEGRAPH_DIR" reset --hard v1.0 2>/dev/null || true
    }
else
    echo "[INFO] Cloning FlameGraph v1.0 to $FLAMEGRAPH_DIR..."
    mkdir -p "$FLAMEGRAPH_DIR"
    git clone --depth=1 --branch v1.0 https://github.com/brendangregg/FlameGraph.git "$FLAMEGRAPH_DIR" 2>/dev/null || {
